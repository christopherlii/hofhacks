 ...
 */
import { generateEdgeId } from '../utils/ids.js';
export class ContextGraphBuilder {
    ...
    weightedConfidence(existing, incoming) {
        ...
        for (const edge1 of this.edges.values()) {
            for (const edge2 of this.edges.values()) {
                ...
                if (edge1.source === edge2.source) {
                    if ((edge1.type === 'prefers' && edge2.type === 'avoids') ||
    ...
    findGaps() {
        ...
        if (isolatedNodes.length > 0) {
            gaps.push({
                ...
                suggestedQuestions: isolatedNodes.slice(0, 5).map(n => `How does "${n.label}" relate to other aspects of their life?`)
            });
        }
        return gaps;
    }
