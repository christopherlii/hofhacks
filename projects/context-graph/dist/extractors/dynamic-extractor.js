...
export class DynamicExtractor {
    ...
    totalTokensUsed = 0;
    estimatedCost = 0;
    constructor(registry, apiKey, model = 'claude-sonnet-4-20250514') {
        ...
        for (const entity of parsed.entities || []) {
            ...
            if (entity.isNewType && entity.newTypeDefinition) {
                ...
                if (resolved.isNew) {
                    ...
                    const registered = this.registry.registerNodeType({
                        id: entity.type,
                        label: entity.type.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()),
        ...
        for (const rel of parsed.relationships || []) {
            ...
            if (rel.isNewType && rel.newTypeDefinition) {
                ...
                if (resolved.isNew) {
                    const registered = this.registry.registerEdgeType({
                        id: rel.type,
                        label: rel.type.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()),
