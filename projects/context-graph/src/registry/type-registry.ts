...
export class TypeRegistryManager {
  ...
  private loadOrCreate(): TypeRegistry {
    ...
    const nodeTypes: Record<string, TypeDefinition> = {};
   ...
   */
  resolveOrPropose(typeName: string, isEdgeType: boolean = false): {
    id: string;
    isNew: boolean;
   ...
   */
  recordUsage(typeId: string, isEdgeType: boolean = false): void {
    const types = isEdgeType ? this.registry.edgeTypes : this.registry.nodeTypes;
    const normalized = normalizeTypeName(typeId);
   ...
   */
  async consolidateTypes(): Promise<{
    mergedNodeTypes: Array<{ from: string[]; to: string }>;
    mergedEdgeTypes: Array<{ from: string[]; to: string }>;
  }> {
...
If no merges needed, return empty arrays.`;
    ...
    const mergedNodeTypes: Array<{ from: string[]; to: string }> = [];
    const mergedEdgeTypes: Array<{ from: string[]; to: string }> = [];
