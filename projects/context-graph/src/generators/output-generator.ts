...
import { ContextGraph, Node, Edge, Cluster, Gap } from '../types/graph.js';
...
export class OutputGenerator {
   ...
   */
  generateMermaid(graph: ContextGraph, maxNodes: number = 30): string {
    const lines: string[] = [];
    lines.push('```mermaid');
    lines.push('graph TB');
    lines.push('');
  ...
  private getNodesByType(graph: ContextGraph, types: string[]): Node[] {
